...
describe("Azure ResourceManager MultiService", () => {
  ...
  describe("Compute VirtualMachines", () => {
    it("should get virtual machine", async () => {
      const result = await client.virtualMachines.get(resourceGroup, "vm1");
  ...
  describe("ComputeDisk Disks", () => {
    it("should get disk", async () => {
      const result = await client.disks.get(resourceGroup, "disk1");
    ...
    it("should create or update disk", async () => {
      const result = await client.disks.createOrUpdate(resourceGroup, "disk1", {
        location,
        properties: {}
      });
      assert.strictEqual(result.name, "disk1");
