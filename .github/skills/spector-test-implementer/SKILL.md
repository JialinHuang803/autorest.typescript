...
Do **not** commit these files (they are generated at runtime and must be excluded):

- `spec-coverage.json` (anywhere in the repo) — generated by the test server, gitignored at repo root
- `coverage/` — test coverage output directory
- The root `.gitignore` — do NOT modify the repository's root `.gitignore` file
